...
#Measured from servo centers, in meters
#Longer arm
ELBOW_WRIST_LENGTH = 0.1850
#Shorter arm
#ELBOW_WRIST_LENGTH = 0.0873

#Measured from wrist servo center to tip of gripper
WRIST_ENDPOINT_LENGTH = 0.0873
...
class AL5D(object):
    ...
    def park(self):
	"""Moves all servos to parking position"""
	with self.ssc32.move_group():
	     self.base(math.pi/2)
	     self.shoulder(math.pi/-4)
	     self.elbow(math.pi*3/4)
	     self.wrist(0)
	     self.gripper(50)

    def circle(self):
	"""Moves the endpoint in a circle manner along the y-z plane"""
	radius = 0.06
	angle = 0.0
	while angle < 360.0 :
		yaxis = (radius * math.sin(math.pi/180.0*angle)) + 0.3
		zaxis = (radius * math.cos(math.pi/180.0*angle)) + 0.2
		angle = angle + 20.0
		time.sleep(1)
		self.move(0, yaxis, zaxis, 0)

    def line(self):
	xaxis = -0.2
	while xaxis < 0.2 :
		self.move(xaxis,0.25,0.1,0)
		xaxis = xaxis + 0.0005
		time.sleep(0.01)
